...
class _TermoPageState extends State<TermoPage> {
  ...
  Future<void> verificarTermo() async {
    ...
    final result = sharedPreferences.getString('data');
    final user = UserPromote.fromMap(jsonDecode(result!)['data']);
    final response = await http.post(Uri.parse(ConstsApi.termoAceite),
        headers: <String, String>{
          'Content-Type': 'application/json',
          'authorization': ConstsApi.basicAuth,
        },
    ...
    if (response.statusCode == 200) {
      ...
      if (data['data'] == "O termo já foi aceito.") {
        Navigator.pushAndRemoveUntil(
          ...
          MaterialPageRoute(
            builder: (context) =>
                const HomePage(initialHomePage: InitialHomePage.escala),
          ),
          (r) => false,
        );
      } else {
              ...
              ? Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                          'Atualizado em ${DateFormat('dd/MM/yyyy').format(DateTime.parse(responseData['dtReferencia']))}',
                          style: GoogleFonts.dosis(
                            textStyle: TextStyle(
                              fontSize: 18,
                              fontWeight: FontWeight.bold,
